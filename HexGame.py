...
def main():
    #default the size of the board to be 11
    game = HexGame(11)
...
class HexGame(object):
    ...
    def reset(self):
        ...
        self.turn = 1 # +1 for black; -1 for red

    def playOneGame(self, gameNumber, player1, player2, gw, window):
        """
        Plays a single game and returns the winner
        ...
        while True:
            ...
            #get the current move for the player
            row, col = cur_player.getMove(self.board)

            #set the board
            self.board[row][col] = self.turn

            #toggle the board position in the simulation and update the window
            gw.gameGrid.mode = self.turn
            gw.gameGrid.toggleColor(gw.gameGrid.label[row][col])
            window.update()    

            #display the winner for the current position        
    ...
    def countStronglyConnectedPositions(self, board, side):
        """
        Count strongly connected positions
        @param board - the current board
        @param side - the current player side
        """
        queue = FIFO_Queue()
